import { _makeAttributes } from './message-view';

describe('_makeAttributes', () => {
  it('presents strings and numbers as-is', () => {
    const str = _makeAttributes({ type: 'string' }, { indent: 4, name: 'a-string' });
    expect(str).toEqual([[4, 'a-string: string']]);

    const num = _makeAttributes({ type: 'number' }, { indent: 2, name: 'a-number' });
